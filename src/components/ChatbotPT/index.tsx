...
import zap from '../../assets/images/whatsapp.svg'
    ...
    opcoes: [
      {
        texto: "WhatsApp",
        linkExterno:
        'https://api.whatsapp.com/send/?phone=5534999306776&text=Ol%C3%A1%2C+gostaria+de+contratar+os+servi%C3%A7os+da+Transtri%C3%A2ngulo+vim+pelo+site+de+voc%C3%AAs.+Voc%C3%AA+pode+me+passar+mais+informa%C3%A7%C3%B5es%3F&app_absent=0',
      },
...
export const Chatbot = () => {
  ...
  const opcoes =
    ...
    perguntaAtual.opcoes.map((opcao, index) => (
      <button
        ...
        className="bg-[#14539a] h-[60px] lg:h-[80px] flex items-center justify-center p-3 lg:p-5 rounded-xl text-white text-sm lg:text-lg shadow-2xl"
      >
        {opcao.texto}
      </button>
    ));
  ...
  if (isRespostaCorreta) {
    return (
      ...
      <div className="fixed bottom-3 right-3 z-[100] bg-[#e3e2e2]  w-[80%] max-w-[500px] max-h-[700px] h-[500px] sm:w-[60%] md:w-[50%] lg:w-[45%] xl:w-[35%]  2xl:w-[30%] lg:h-[600px] 2xl:h-[650px] rounded-xl flex flex-col shadow-2xl border-2 ">
        <header className="bg-[#074d2c] h-[80px] p-5 flex justify-between rounded-t-xl sticky top-0  shadow-2xl ">
          ...
          <div className="historico">
            {historicoPerguntas.map((item, index) => (
              ...
              >
                ...
                <p className="text-base lg:text-lg bg-[#14539a] p-3 lg:p-5 rounded-t-2xl rounded-s-2xl !text-[#ffffff]  self-end ">
                  {item.resposta}
                </p>
              </div>
            ))}
        ...
        <footer className="text-center text-sm lg:text-base bg-[#074d2c] !text-white h-[80px] w-full p-5 rounded-b-xl">
          {historico.length > 0 && (
            <button
              onClick={handleVoltar}
              className="bg-[#14539a] text-sm lg:text-base p-2 lg:px-3 lg:py-3 rounded-md text-white flex gap-2 justify-between items-center" 
            >
              <div>
  ...
  return (
    <>
      {isButtonVisible && (
        ...
        <div className="fixed z-[100] bottom-24 right-3 cursor-pointer ">
          <SpeedDial>
      ...
      {isCardVisible && (
        <div
          ...
          className="fixed bottom-3 right-3 z-[1000] bg-[#e3e2e2] w-[80%] max-w-[500px] max-h-[700px] h-[500px] sm:w-[60%] md:w-[50%] lg:w-[45%] xl:w-[35%]  2xl:w-[30%] lg:h-[600px] 2xl:h-[650px] rounded-xl flex flex-col shadow-2xl border-2 "
        >
          <header className="bg-[#074d2c] h-[80px] p-5 flex justify-between rounded-t-xl sticky top-0  shadow-xl">
            <img src={Logo} alt="BMOUSE SAÚDE" className="w-[120px]" />
            <button onClick={handleButtonClose}>
              {" "}
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
          ...
          <div className="h-[500px] lg:h-[700px] bg-[#e3e2e2] p-5 text-center flex flex-col gap-2 justify-between py-3 lg:py-5 overflow-x-auto">
            ...
            <button
              className="bg-[#14539a] shadow-2xl p-2 lg:px-3 lg:py-3 w-full rounded-xl hover:opacity-90 text-white"
